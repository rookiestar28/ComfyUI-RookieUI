...
def _warning_messages_from_codes(codes: list[str]) -> list[str]:
    return [
        _ADETAILER_WARNING_MESSAGES[code]
        for code in codes
        if code in _ADETAILER_WARNING_MESSAGES
    ]
...
def _build_detector_entries() -> tuple[list[dict[str, object]], str]:
    ...
    ultralytics_models = [
        selector
        for selector in inventory.ultralytics
...
def build_adetailer_catalog_payload() -> dict[str, object]:
    ...
    # IMPORTANT: keep this payload Comfy-native and inventory-backed; F78/F79 must build on this
    # shared seam instead of reintroducing A1111 script-owned detector/runtime state.
    return {
        ...
        "controlnet_default_module": str(controlnet_modules.get("default_module", _ADETAILER_CONTROLNET_DEFAULT_MODULE)),
...
def _normalize_controlnet_block(
    ...
    *,
    ...
    module = normalize_option_label(
        raw_block.get("module"),
        f"{field_prefix}.module",
        max_length=128,
    ) or _ADETAILER_CONTROLNET_DEFAULT_MODULE
    ...
    guidance_start = round(
        _coerce_float(raw_block.get("guidance_start", 0.0), f"{field_prefix}.guidance_start"),
        3,
    )
    guidance_end = round(
        _coerce_float(raw_block.get("guidance_end", 1.0), f"{field_prefix}.guidance_end"),
        3,
    )
...
def normalize_adetailer_payload(
    ...
    *,
    ...
    for index in range(ADETAILER_INTEGRATED_DEFAULT_UNIT_COUNT):
        ...
        detector_classes = normalize_option_label(
            raw_unit.get("detector_classes"),
            f"{field_prefix}.detector_classes",
            max_length=256,
        )
        ...
        mask_min_ratio = round(
            _coerce_float(raw_unit.get("mask_min_ratio", 0.0), f"{field_prefix}.mask_min_ratio"),
            4,
        )
        mask_max_ratio = round(
            _coerce_float(raw_unit.get("mask_max_ratio", 1.0), f"{field_prefix}.mask_max_ratio"),
            4,
        )
        ...
        denoising_strength = round(
            _coerce_float(raw_unit.get("denoising_strength", 0.4), f"{field_prefix}.denoising_strength"),
            3,
        )
        ...
        inpaint_only_masked = _coerce_bool(
            raw_unit.get("inpaint_only_masked", True),
            f"{field_prefix}.inpaint_only_masked",
            strict=False,
        )
        ...
        use_checkpoint = _coerce_bool(
            raw_unit.get("use_checkpoint", False),
            f"{field_prefix}.use_checkpoint",
            strict=False,
        )
        ...
        scheduler_name = normalize_option_label(
            raw_unit.get("scheduler_name"),
            f"{field_prefix}.scheduler_name",
            max_length=96,
        ) or "Use same scheduler"
        ...
        noise_multiplier = round(
            _coerce_float(raw_unit.get("noise_multiplier", 1.0), f"{field_prefix}.noise_multiplier"),
            2,
        )
        ...
        units.append(
            NormalizedADetailerUnitRequest(
                ...
                prompt=normalize_prompt_text(raw_unit.get("prompt", ""), f"{field_prefix}.prompt"),
                negative_prompt=normalize_prompt_text(
                    raw_unit.get("negative_prompt", ""),
                    f"{field_prefix}.negative_prompt",
                ),
                ...
                prompt_uses_main=not bool(normalize_prompt_text(raw_unit.get("prompt", ""), f"{field_prefix}.prompt")),
                negative_prompt_uses_main=not bool(
                    normalize_prompt_text(raw_unit.get("negative_prompt", ""), f"{field_prefix}.negative_prompt")
                ),
