...
class ModelFamilyRegistryTests(unittest.TestCase):
    def test_candidate_supported_sources_cover_every_non_sd_registry_entry(self) -> None:
        ...
        self.assertEqual(OFFICIAL_TEMPLATE_SOURCE_VERSION, "0.11.43")
    ...
    def test_official_template_manifest_uses_current_workflow_template_basis(self) -> None:
        ...
        self.assertEqual(
            ...
            "comfyui-workflow-templates-json==0.1.49:image_krea2_turbo_t2i.json",
        )
        packaged_paths = [path for path in source_paths.values() if path.startswith("comfyui-workflow")]
        self.assertTrue(packaged_paths)
        self.assertTrue(
            all(path.startswith("comfyui-workflow-templates-json==0.1.49:") for path in packaged_paths)
        )
        self.assertEqual(
    ...
    def test_registry_tracks_translation_and_public_family_separately(self) -> None:
        ...
        self.assertEqual(krea2_entry.default_template_lora_trigger_word, "monochrome ink wash style")
        self.assertTrue(krea2_entry.prompt_enhancement_visible)
        self.assertTrue(krea2_entry.default_prompt_enhancement_enabled)
    ...
    def test_presets_use_public_family_identity_for_newer_families(self) -> None:
        ...
        self.assertEqual(krea2_preset["template_lora_trigger_word"], "monochrome ink wash style")
        self.assertTrue(krea2_preset["prompt_enhancement_enabled"])
