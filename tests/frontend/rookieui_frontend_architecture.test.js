...
import { ROOKIEUI_ASSET_REVISION } from "../../web/rookieui_asset_revision.js";
  ...
  const walk = (currentDir) => {
    for (const entry of fs.readdirSync(currentDir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        // IMPORTANT: defence in depth, not the layout constraint itself. Test assets now live
        // outside the served tree, and the Python suite asserts that they stay there. This skip
        // only keeps the byte and line budgets honest in a window where someone recreates a
        // tests directory under the served tree before that guard runs. Removing it as dead
        // code turns a harmless belt-and-braces check into a gap; removing the Python guard
        // instead, on the grounds that this line already covers it, would put test files back
        // in front of every browser while every budget measured here still passed.
