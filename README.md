...
- Refreshed the tested ComfyUI Core, frontend, Desktop, and official workflow-template source basis, including template coverage through `comfyui-workflow-templates` 0.11.43.
...
- The official template manifest is aligned to `comfyui-workflow-templates` 0.11.43; shipped support remains limited to the RookieUI profiles listed below.
...
- Current official-template alignment uses `comfyui-workflow-templates` 0.11.43 as the source basis for refreshed host blueprints and packaged gallery JSON assets, but workflow families outside the shipped profile list are not implied support.
