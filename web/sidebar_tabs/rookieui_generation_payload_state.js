...
export function buildTxt2ImgPayloadFromElements(elements) {
  return {
    ...
    template_lora_enabled: elements.templateLoraEnabled.checked,
    template_lora_strength: Number(elements.templateLoraStrength.value),
    template_lora_trigger_word: elements.templateLoraTriggerWord.value,
...
export function buildImg2ImgPayloadFromElements(elements, imageEditReferencePayload = {}) {
  const qwen21Edit = elements.profileState.value === "qwen_image_21_edit";
  return {
    prompt: elements.prompt.value,
    negative_prompt: elements.negativePrompt.disabled ? "" : elements.negativePrompt.value,
    profile: elements.profileState.value,
    dtype_profile: qwen21Edit ? "automatic" : elements.lowBits.value,
    checkpoint_name: elements.checkpoint.value,
    vae_name: elements.vae.value,
    text_encoder_name: elements.textEncoder.value,
    image_asset: elements.imageAsset.value,
    image_data: elements.imageData.value,
    mask_asset: qwen21Edit ? "" : elements.maskAsset.value,
    mask_data: qwen21Edit ? "" : elements.maskData.value,
    ...
    reference_resolution: qwen21Edit ? Number(elements.referenceResolution.value) : 0,
    output_size_mode: qwen21Edit ? elements.outputSizeMode.value : "reference",
    edit_task: qwen21Edit ? elements.editTask.value : "edit",
    mode: elements.mode.value,
    batch_images: qwen21Edit ? [] : parseStringArrayField(elements.batchImagesData?.value ?? "[]"),
    width: Number(elements.width.value),
    height: Number(elements.height.value),
    resize_mode: elements.resizeMode.value,
    steps: Number(elements.steps.value),
    cfg_scale: Number(elements.cfgScale.value),
    shift: qwen21Edit ? null : readOptionalNumericValue(elements.shift),
    flux_guidance: qwen21Edit ? null : readOptionalNumericValue(elements.fluxGuidance),
    edit_megapixels: qwen21Edit ? null : readOptionalNumericValue(elements.editMegapixels),
    sampler_name: elements.sampler.value,
    scheduler_name: elements.scheduler.disabled ? "" : elements.scheduler.value,
    prompt_enhancement_enabled: qwen21Edit ? false : elements.promptEnhancementEnabled.checked,
    seed: Number(elements.seed.value),
    seed_extra: elements.seedExtra.checked,
    batch_size: qwen21Edit ? 1 : Number(elements.batchSize.value),
    clip_skip: Number(elements.clipSkip.value),
    denoise_strength: qwen21Edit ? 1 : Number(elements.denoiseStrength.value),
    ...
    hires_enabled: qwen21Edit ? false : elements.hiresEnabled.checked,
    hires_scale: Number(elements.hiresScale.value),
    hires_steps: Number(elements.hiresSteps.value),
    hires_denoise: Number(elements.hiresDenoise.value),
    hires_upscale_method: elements.hiresUpscaleMethod.value,
    template_lora_name: qwen21Edit ? "" : elements.templateLoraName.value,
    lora_name: qwen21Edit ? "" : elements.loraName.value,
    lora_strength_model: Number(elements.loraStrengthModel.value),
    lora_strength_clip: Number(elements.loraStrengthClip.value),
    adetailer: qwen21Edit ? {} : parseJsonObjectField(elements.adetailer?.value ?? "{}", {}),
    controlnet_units: qwen21Edit ? [] : parseJsonObjectArrayField(elements.controlnetUnits?.value ?? "[]"),
  };
}
