...
echo "[pre-push] Step 1/6: detect-secrets"
"$VENV_PY" -m pre_commit run detect-secrets --all-files

echo "[pre-push] Step 2/6: pre-commit all hooks"
capture_precommit_snapshots
...
echo "[pre-push] Step 3/6: prompt compiler guard tests"
...
echo "[pre-push] Step 4/6: backend unit tests"
MOLTBOT_STATE_DIR="$ROOT_DIR/moltbot_state/_local_unit" \
  "$VENV_PY" scripts/run_unittests.py --start-dir tests --pattern "test_*.py"

echo "[pre-push] Step 5/6: frontend type validation + test suite"
npm run test:types
npm test

if [ "${ROOKIEUI_RUN_LIVE_SMOKE:-0}" = "1" ]; then
  echo "[pre-push] Step 6/6: optional host-embedded E2E lane"
  "$VENV_PY" scripts/run_host_embedded_e2e.py
else
  echo "[pre-push] Step 6/6: optional host-embedded E2E lane skipped (set ROOKIEUI_RUN_LIVE_SMOKE=1 to enable)"
fi
